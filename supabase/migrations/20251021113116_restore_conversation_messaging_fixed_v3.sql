/*
  ...
  ## Changes Made
  1. Creates conversations table with anonymity tracking
  2. Modifies messages table to use conversation_id instead of receiver_id
  3. Renames sent_at to created_at and seen_at to read_at for consistency
  4. Creates RPC functions for conversation management
  5. Updates RLS policies for conversation-based access
*/

-- ============================================================================
-- 1. DROP ALL OLD FUNCTIONS AND POLICIES FIRST
-- ============================================================================
...
-- Rename seen_at to read_at if needed
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    ALTER TABLE public.messages RENAME COLUMN seen_at TO read_at;
  END IF;
END $$;
...
-- Make conversation_id NOT NULL only if all messages have it
DO $$
BEGIN
  IF NOT EXISTS (
...
BEGIN
  ...
  IF EXISTS (
    ...
    ALTER TABLE public.messages DROP COLUMN receiver_id CASCADE;
  END IF;
END $$;
...
CREATE FUNCTION public.mark_conversation_delivered(conv_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
